  ...
  return (
    <div className="flex min-h-screen" style={{ backgroundColor: '#2B3655' }}>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {fetchError ? (
              <div className="text-center py-20">
                <p className="font-cormorant text-2xl mb-4" style={{ color: 'rgba(247, 245, 240, 0.7)' }}>
                  {fetchError}
                </p>
                <button
                  onClick={fetchArticles}
                  className="font-montserrat text-sm tracking-wider uppercase transition-opacity hover:opacity-70"
                  style={{ color: 'var(--color-gold)' }}
                >
                  Try Again
                </button>
              ...
              <>
                {featuredArticle && (
                  <article
                    className="mb-8 p-12 lg:p-16 cursor-pointer transition-all hover:shadow-lg"
                  ...
                  >
                    ...
                    <p className="font-montserrat text-sm text-center mt-6 tracking-wider" style={{ color: 'var(--color-gold)' }}>
                      READ MORE
                    </p>
                  </article>
                )}
                ...
                {nextThree.length > 0 && (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    {nextThree.map((article) => (
                      <article
                        key={article.id}
                        className="p-8 flex flex-col min-h-[250px] cursor-pointer transition-all hover:shadow-lg"
                        style={{ backgroundColor: 'var(--color-ivory)' }}
                        onClick={() => setSelectedArticle(article)}
                      >
                        <h3 className="font-cormorant text-2xl mb-4" style={{ color: 'var(--color-navy)' }}>
                          {article.title}
                        </h3>
                        <p className="font-baskerville text-xs mb-4" style={{ color: 'var(--color-text-dark)' }}>
                          {formatDate(article.created_at)}
                        </p>
                        {article.opening_line && (
                          <p className="font-baskerville text-sm leading-relaxed line-clamp-4" style={{ color: 'var(--color-text-dark)' }}>
                            {article.opening_line}
                          </p>
                        )}
                        <div className="flex-grow" />
                        <p className="font-montserrat text-xs mt-4 tracking-wider" style={{ color: 'var(--color-gold)' }}>
                          READ MORE
                        </p>
                      </article>
                    ))}
      ...
      {selectedArticle && (
        ...
        >
          <div
            className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto p-8 lg:p-16"
            style={{ backgroundColor: 'var(--color-ivory)' }}
          ...
          >
            ...
            <h2 className="font-cormorant text-3xl md:text-4xl lg:text-5xl mb-6 leading-tight" style={{ color: 'var(--color-navy)', textTransform: 'uppercase' }}>
              {selectedArticle.title}
            </h2>

            <p className="font-baskerville text-sm mb-8" style={{ color: 'var(--color-text-dark)' }}>
              {formatDate(selectedArticle.created_at)}
            </p>

            {selectedArticle.category && (
              <p className="font-montserrat text-xs tracking-widest uppercase mb-8" style={{ color: 'var(--color-gold)' }}>
                {selectedArticle.category}
              </p>
            )}
            ...
            <div
              className="font-baskerville text-base leading-relaxed whitespace-pre-wrap"
