  ...
  const handleAdminSignup = async (e: FormEvent) => {
    ...
    try {
      ...
      if (authData.user) {
        ...
        const { error: profileError } = await supabase.from('profiles').update({ full_name: fullName, role: 'admin' }).eq('id', authData.user.id);
    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-2/5 flex-col items-center justify-center p-12 relative" style={{ backgroundColor: 'var(--color-navy)' }}>
        <div className="flex flex-col items-center mb-auto mt-20">
          <Link to="/" className="flex flex-col items-center">
            ...
            <p className="font-montserrat text-xs tracking-[0.4em] uppercase" style={{ color: 'var(--color-gold)' }}>Pipeline. Intelligence. Engineered.</p>
          </Link>
        </div>
        <div className="mt-auto mb-20 text-center">
          <p className="font-montserrat text-sm mb-2" style={{ color: 'rgba(255, 255, 255, 0.7)' }}>Admin Account Creation</p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-3/5 flex items-center justify-center p-6 md:p-12" style={{ backgroundColor: 'var(--color-ivory)' }}>
        <div className="max-w-md w-full">
          ...
          <h2 className="font-cormorant text-4xl md:text-5xl mb-3" style={{ color: 'var(--color-text-dark)' }}>Admin Sign-Up</h2>
          <p className="font-baskerville text-base mb-10" style={{ color: 'var(--color-text-muted)' }}>Create your administrator account with full system access.</p>
          <form onSubmit={handleAdminSignup} className="space-y-6">
            ...
            <button type="submit" disabled={loading} className="w-full font-montserrat text-xs tracking-[0.25em] uppercase py-5 transition-all duration-300 mt-8" style={{ backgroundColor: loading ? 'var(--color-text-muted)' : 'var(--color-navy)', color: 'var(--color-white)' }} onMouseEnter={(e) => { if (!loading) e.currentTarget.style.backgroundColor = 'var(--color-gold)'; }} onMouseLeave={(e) => { if (!loading) e.currentTarget.style.backgroundColor = 'var(--color-navy)'; }}>{loading ? 'Creating Account...' : 'Create Admin Account'}</button>
            <div className="h-px w-full my-8" style={{ backgroundColor: 'rgba(27, 35, 64, 0.2)' }} />
            <div className="text-center">
              <Link to="/login" className="font-montserrat text-xs transition-colors" style={{ color: 'var(--color-gold)' }}>Back to Login</Link>
            </div>
          </form>
        </div>
      </div>
